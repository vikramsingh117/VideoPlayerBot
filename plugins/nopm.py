...
from config import API_ID, API_HASH, SESSION_STRING, REPLY_MESSAGE, OLD_PMS, BOT_TOKEN

User = Client(
    BOT_TOKEN,  # ✅ Now this runs as a bot
    API_ID,
    API_HASH
)

# ✅ Removed filters.edited (not available in Pyrogram 2.x)
...
async def nopm(client, message):
    if REPLY_MESSAGE:
        try:
            ...
            print(f"Error in nopm handler: {e}")  # ✅ Better error logging
